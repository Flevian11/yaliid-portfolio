...
class PortfolioController extends Controller {
 ...
 public function project(Project $project): JsonResponse { abort_unless($project->is_published,404); return response()->json($project->load(['features','links','media','technologies'])); }
 public function services(): JsonResponse { return response()->json(Service::where('is_published',true)->orderBy('display_order')->get()); }
 public function cv(): JsonResponse { $p=Profile::where('is_active',true)->firstOrFail(); return response()->json(['profile'=>$p,'experiences'=>Experience::with('achievements')->where('is_published',true)->orderBy('display_order')->get(),'education'=>Education::where('is_published',true)->orderBy('display_order')->get(),'certifications'=>Certification::where('is_published',true)->orderBy('display_order')->get(),'skills'=>SkillCategory::with('skills')->where('is_published',true)->orderBy('display_order')->get()]); }
}
